...
$(function() {
  ...
  $("#inscripcion a:contains('Inscripcion')").parent().addClass('active');
...
$( document ).ready(function() {
  // Se ejecuta cuando se han cargado toda la pagina
  $("#txtUsuario").hover(function(){
    $('#txtUsuario').tooltip('show');
  });
...
if (localStorage.nomUsu!=""){
	var txtUsuario=document.getElementById("txtUsuario");
	txtUsuario.value=localStorage.nomUsu;
}
if (localStorage.passUsu!=""){
	var txtPassword=document.getElementById("txtPassword");
	txtPassword.value=localStorage.passUsu;
}



function comprobar(){
//leer elementos de la pagina
var txtUsuario=document.getElementById("txtUsuario");
var txtPassword=document.getElementById("txtPassword");
//comprobar usuario
var reg = new RegExp('/^([\da-z_\.-]+)@([\da-z\.-]+)\.([a-z\.]{2,6})$/');
if (txtNombre.value==""){
	//alert("El usuario debes ser su email");
  //$('#titModal').text=
  showMensaje("Falta Nombre","Debes escribir el nombre")
  //$('#modal').modal('show');
	txtNombre.focus();
	return;
}
//comprobar password
if (txtCorreo.value.==""){
	showMensaje("Falta Correo","Debes escribir el correo")
...
//comprobar q tiene algún digito
reg = new RegExp('[0-9]+');
if (!reg.test(txtPassword.value)){
	alert("El password tiene que tener algún dígito");
	txtPassword.focus();
	return;
}
//guardar en local el nombre de usuario
localStorage.nomUsu=txtUsuario.value;
//si recordar seleccionado guardar contraseña
var chkRecordar=document.getElementById("chkRecordar");
if (chkRecordar.checked){
	localStorage.passUsu=md5(txtPassword.value);
}

}
//funcion que muestra mediante una ventana modal un titulo y
//un mensaje
